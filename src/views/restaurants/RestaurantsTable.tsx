...
import {deleteRestaurant, getRestaurants} from "../../services/restaurant.service.ts";
import {
        ...
        <div style={{ padding: '20px' }}>
            <CForm className="filter-container mb-3">
                <CRow className="align-items-end">
                    ...
                    <CCol md="3" className="text-md-end">
                        <CTooltip content={t('restaurantTable.applyFilters')} placement="top">
                            <CButton onClick={handleFilterApply} color="primary" className="me-2 mb-2">
                                <CIcon icon={cilFilter} />
                            </CButton>
                        </CTooltip>
                        <CTooltip content={t('restaurantTable.resetFilters')} placement="top">
                            <CButton onClick={handleFilterReset} color="secondary" className="mb-2">
            ...
            <CTable striped>
                <CTableHead>
                    <CTableRow>
                        <CTableHeaderCell>
                            {t('restaurantTable.id')}
                        </CTableHeaderCell>
                        <CTableHeaderCell >
                            {t('restaurantTable.name')}
                        </CTableHeaderCell>
                        <CTableHeaderCell >
                            {t('restaurantTable.cuisine')}
                        </CTableHeaderCell>
                        <CTableHeaderCell >
                ...
                <CTableBody>
                    {restaurants.map((restaurant) => (
                        <CTableRow key={restaurant.id}>
                            ...
                            <CTableDataCell>{new Date(restaurant.createdAt).toLocaleString()}</CTableDataCell>
                            <CTableDataCell>{new Date(restaurant.updatedAt).toLocaleString()}</CTableDataCell>
                            <CTableDataCell>
                                <div className="d-flex">

                                        <CTooltip content={t('restaurantTable.editProject')} placement="top">
                                            <div style={tooltipStyle}>
                                                <CButton color="warning" onClick={() => handleEditClick(restaurant.id)} className="me-2">
                                                    <CIcon icon={cilPencil} />
                                                </CButton>
                                            </div>
                                        </CTooltip>
                                        <CTooltip content={t('restaurantTable.deleteProject')} placement="top">
                                            <div style={tooltipStyle}>
                                                <CButton color="danger" onClick={() => handleDeleteClick(restaurant.id)} className="me-2">
                                                    <CIcon icon={cilTrash} />
                                                </CButton>
                                            </div>
                                        </CTooltip>
            ...
            <CButtonGroup className='me-3'>
                <CTooltip content={t('restaurantTable.previousPage')} placement="top">
                    <div style={tooltipStyle}>
                        <CButton color="primary" onClick={() => setPage(page - 1)} disabled={page === 1}>
                ...
                <CTooltip content={t('restaurantTable.nextPage')} placement="top">
                    <div style={tooltipStyle}>
                        <CButton color="primary" onClick={() => setPage(page + 1)} disabled={page === totalPages}>
